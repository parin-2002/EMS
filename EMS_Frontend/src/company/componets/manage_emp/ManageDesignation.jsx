...
import { Designation } from "../manage_emp/Addemp";
...
async function deleteDesignation(id, getDesignation) {
  try {
    if (confirm("Are you sure?")) {
      notifyInfo("Request is sent to server it may take sometime");
      const result = await instance.delete(
        `/api/company/manageemp/deletedesignation?id=${id}`,
        {
...
function UpdateDesignation({ getDesignation, data }) {
    ...
    onSubmit: async (values) => {
      try {
        ...
        const result = await instance.put(
          "/api/company/manageemp/updatedesignation",
          values,
          {
            withCredentials: true,
          }
    ...
    <div>
      ...
      <Dialog open={open} onClose={handleClose}>
        <form onSubmit={formik.handleSubmit}>
          <DialogTitle className="px-5 mx-5">Update Designation</DialogTitle>
          <DialogContent>
            <div className="mt-2">
              <TextField
                variant="standard"
...
function ManageDesignation({ getDesignation }) {
  const data = useSelector((state) => state.designation.value);
        ...
        Cell: ({ cell }) => {
          return (
            <DeleteIcon
              className="delete"
              onClick={() => {
                deleteDesignation(cell.row.original._id, getDesignation);
              }}
            />
          );
        },
        ...
        >
          <TableHead className="sticky-top">
            <TableRow>
              ...
              >
                <AccountBoxIcon className="mb-1" /> Designation
              </TableCell>
